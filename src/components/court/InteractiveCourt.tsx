 ...
 *
 * Features:
 * — responsive viewBox scaling (400×640, xMidYMid meet)
 ...
 * — full court with both halves: top = offense/front court, bottom = defense/back court
 * — half prop controls child positioning: 'top' = as-is, 'bottom' = translate +320
 */
...
const animationStyles = `
  ...
  @keyframes ic-dash-move { to { stroke-dashoffset: -40; } }
  @keyframes ic-dash-shoot { to { stroke-dashoffset: -60; } }
  ...
  .ic-zone-open { animation: ic-pulse-ring-green 2s ease-in-out infinite; transform-origin: center; }
  .ic-zone-pressure { animation: ic-pulse-ring-red 1.5s ease-in-out infinite; transform-origin: center; }
  .ic-dash-pass { stroke-dasharray: 8 6; animation: ic-dash-move 0.8s linear infinite; }
  .ic-dash-shoot { stroke-dasharray: 10 8; animation: ic-dash-shoot 1s linear infinite; }
  .ic-glow { animation: ic-glow-pulse 2s ease-in-out infinite; }
  .ic-player-draggable { cursor: grab; }
  .ic-player-draggable:active { cursor: grabbing; }
  .ic-player-hero-glow { animation: ic-glow-pulse 1.5s ease-in-out infinite; }
`;

/* ------------------------------------------------------------------ */
/*  Court Half Renderer — correct 3pt/ft/paint order                  */
/* ------------------------------------------------------------------ */
...
function PlayerOffenseSVG({ x, y, size = 22, label = '', draggable = false, isDragging = false }: {
  x: number; y: number; size?: number; label?: string; draggable?: boolean; isDragging?: boolean;
}) {
  const r = size;
  return (
...
function PlayerDefenseSVG({ x, y, size = 20, label = '', draggable = false, isDragging = false }: {
  x: number; y: number; size?: number; label?: string; draggable?: boolean; isDragging?: boolean;
}) {
  const r = size;
  return (
...
function PlayerHeroSVG({ x, y, size = 24, label = '', draggable = false, isDragging = false }: {
  x: number; y: number; size?: number; label?: string; draggable?: boolean; isDragging?: boolean;
}) {
  const r = size;
  return (
...
/* ------------------------------------------------------------------ */
/*  Court Background — full court, both halves                        */
/* ------------------------------------------------------------------ */
...
function CourtBackground({ w, h, pad }: { w: number; h: number; pad: number }) {
  ...
  return (
    <g>
      ...
      <circle cx={cx} cy={h / 2} r="30" fill="none" stroke="white" strokeWidth="1.5" strokeOpacity="0.4" strokeDasharray="4,4" />

      {/* TOP HALF — offense / front court */}
      <CourtHalfSVG cx={cx} w={w} baseY={pad + 8} dir={1} />

      {/* BOTTOM HALF — defense / back court */}
      <CourtHalfSVG cx={cx} w={w} baseY={h - pad - 8} dir={-1} />
    </g>
  );
}
...
const InteractiveCourt: React.FC<InteractiveCourtProps> = ({
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    setLocalPlayers(prev => prev.map(p => (p.id === dragId ? { ...p, x: newX, y: newY } : p)));
    e.preventDefault();
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    if (dragId) {
      ...
      if (player && onPlayerDrag) onPlayerDrag(dragId, player.x, player.y);
      setDragId(null);
  ...
  const trajectoryEls = useMemo(() => trajectories.map((t, i) => {
    ...
    return (
      <g key={`traj-${i}`}>
        <path d={`M ${t.from.x} ${t.from.y} Q ${midX} ${midY}, ${t.to.x} ${t.to.y}`} fill="none" stroke={color} strokeWidth="5" opacity="0.08" />
        <path d={`M ${t.from.x} ${t.from.y} Q ${midX} ${midY}, ${t.to.x} ${t.to.y}`} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" className={t.animated !== false ? dashClass : ''} opacity="0.85" />
        {(() => {
          const dx = t.to.x - (midX + (t.to.x - midX) * 0.5);
          const dy = t.to.y - (midY + (t.to.y - midY) * 0.5);
          const angle = Math.atan2(dy, dx) * (180 / Math.PI);
          return <polygon points={`${t.to.x},${t.to.y} ${t.to.x - 8},${t.to.y - 5} ${t.to.x - 8},${t.to.y + 5}`} fill={color} opacity="0.9" transform={`rotate(${angle}, ${t.to.x}, ${t.to.y})`} />;
        })()}
      </g>
    );
  }), [trajectories]);
  ...
  const zoneEls = useMemo(() => zones.map((z, i) => {
    ...
    return (
      <g key={`zone-${i}`}>
        <circle cx={z.x} cy={z.y} r={r * 1.6} fill="none" stroke={color} strokeWidth="3" opacity="0.15" className={cls} />
        <circle cx={z.x} cy={z.y} r={r} fill="none" stroke={color} strokeWidth="2.5" opacity="0.5" strokeDasharray="4,3" />
        <circle cx={z.x} cy={z.y} r="4" fill={color} opacity="0.6" />
        {z.label && <text x={z.x} y={z.y - r - 6} textAnchor="middle" fill={color} fontSize="10" fontWeight="bold" fontFamily="Nunito, sans-serif">{z.label}</text>}
      </g>
    );
  }), [zones]);
  ...
  return (
    ...
    >
      <defs>
        ...
        <filter id="ic-glow-filter"><feGaussianBlur stdDeviation="4" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        <filter id="glow"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        <filter id="softShadow"><feDropShadow dx="0" dy="2" stdDeviation="3" floodOpacity="0.3" /></filter>
        <filter id="successGlow"><feGaussianBlur stdDeviation="6" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        <marker id="arrowYellow" markerWidth="6" markerHeight="6" refX="6" refY="3" orient="auto"><polygon points="0,0 6,3 0,6" fill="#FFE135" /></marker>
        <marker id="arrowGreen" markerWidth="6" markerHeight="6" refX="6" refY="3" orient="auto"><polygon points="0,0 6,3 0,6" fill="#2ECC71" /></marker>
      </defs>

      {/* Court backdrop — full court with both halves */}
      ...
      {/* Children — translate to correct half */}
      {half === 'bottom' ? (
        <g transform={`translate(0, ${height / 2})`}>{children}</g>
      ) : half === 'top' ? (
        <g>{children}</g>
      ) : children}
    </svg>
